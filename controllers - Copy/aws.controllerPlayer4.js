...
exports.doUpload = async (req, res) => {
	...
	if (req.file) {
		...
		var filename = Date.now()+'-OTTP4-'+req.file.originalname;
		params.Key = filename;
		...
		if (getMoviePlayerB1.status == "success") {
		  if (getMoviePlayerB1.result.txt_player4_image) {
			  obj = {
				status: "success",
				message: "Already exist txt_player4_image File!",
				response: getMoviePlayerB1.result.txt_player4_image
			};
		  } else {
			  obj = {
				status: "fail",
