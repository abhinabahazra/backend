...
exports.doUpload = async (req, res) => {
	...
	if (req.file) {
		...
		var filename = Date.now()+'-OTTP5-'+req.file.originalname;
		params.Key = filename;
		...
		if (getMoviePlayerB1.status == "success") {
		  if (getMoviePlayerB1.result.txt_player5_image) {
			  obj = {
				status: "success",
				message: "Already exist txt_player5_image File!",
				response: getMoviePlayerB1.result.txt_player5_image
			};
		  } else {
			  obj = {
				status: "fail",
