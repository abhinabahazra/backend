...
exports.doUpload = async (req, res) => {
	...
	if (req.file) {
       ...
       // console.log(checkUser);
        ...
        //console.log('51');
        let getMovieLinkB1 = await movieservices.findUserKycById(req.body.num_user_id);
        console.log(getMovieLinkB1);
		if (getMovieLinkB1.status == "success") {
		  if (getMovieLinkB1.result.txt_pancard_link && getMovieLinkB1.result.txt_pancard_link != "" && getMovieLinkB1.result.txt_pancard_link != "undefined") {
			  obj = {
				status: "success",
				message: "Already exist txt_pancard_link File!",
				response: getMovieLinkB1.result.txt_pancard_link
			};
		  } else {
			  obj = {
				status: "fail",
