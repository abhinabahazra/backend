...
app1.post("/otp_check", async (req, res) => {
    ...
    if (required.length === 0) {
        ...
        if (getUserByPhone.status == "success") {
            let status = '1';
            let updatePhoneverify = await userservices.updatePhoneVerifyById(getUserByPhone.result.num_user_id,status);
            getUserByPhone.result.yn_mobile_verified = status;
