...
app1.post('/insert_transactionByWallet', async (req, res, next) => {
  ...
  if (required.length === 0) {
    ...
    if(getUserById.status == "success"){
        ...
        let checkAmout = await movieservices.checkReferTranAmount(req.body.num_user_id);
        let checkDebitAmout = await movieservices.checkDebitTran(req.body.num_user_id);
        let newbalAmount = 0;
        let newCrdAmount = 0;
        // let cur_debit_amount;
        // let cur_balance_amount;
            ...
            if (getTran.status == "success") {
                ...
                // console.log(movieTranUser);
                  ...
                  if (getTranUser.status == "success") {
                    ...
                    let getWalletById=await userservices.findUserAllWalletBalanceById(req.body.num_user_id);
                    // console.log(getWalletById.result.cur_credit_amount);
                    // console.log(getWalletById.result.cur_debit_amount);
                    // console.log(getWalletById.result.cur_balance_amount);

                    //return; 
                    //-------------------------------------->
                    let amount=req.body.num_amount;

                    let cur_debit_amount_new=getWalletById.result.cur_debit_amount;
                    let cur_balance_amount_new=getWalletById.result.cur_balance_amount;
                    let movieTranUser1 = {
                        num_user_id : req.body.num_user_id,
                       // txt_user_id : req.body.txt_user_id,
                        cur_credit_amount:newCrdAmount,
                        cur_debit_amount:cur_debit_amount_new+amount,
                        cur_balance_amount:cur_balance_amount_new-amount,
                        txt_transaction_no : randNumber,
                        dat_update_date : finaldt
                      };
